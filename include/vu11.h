...
void rippled()
{
  ...
  default:                                                                                   // Middle of the ripples.
    ledsLeft[(center + step + N_PIXELS) % N_PIXELS] += CHSV(colour, 255, (MY_FADE / step * 2)); // Simple wrap from Marc Miller.
    ledsLeft[(center - step + N_PIXELS) % N_PIXELS] += CHSV(colour, 255, (MY_FADE / step * 2));
    step++; // Next step.
    break;
    //  }
